package com.mesosphere.sdk.activemq.api;
        ...
        switch(filename) {
            ...
            case "executor.zip":
                ...
                return Response.ok(file, MediaType.APPLICATION_OCTET_STREAM)
                        .header("Content-Disposition", "attachment; filename=\"" + file.getName() + "\"") //optional
                        .build();
            case "jre-8u152-linux-x64.tar.gz":
                file = new File(MESOS_SANDBOX + "/jre-8u152-linux-x64.tar.gz");
                return Response.ok(file, MediaType.APPLICATION_OCTET_STREAM)
                        .header("Content-Disposition", "attachement; filename=\"" + file.getName() + "\"")
                        .build();
            case "libmesos-bundle-master-28f8827.tar.gz":
                file = new File(MESOS_SANDBOX + "/libmesos-bundle-master-28f8827.tar.gz");
                return Response.ok(file, MediaType.APPLICATION_OCTET_STREAM)
                        .header("Content-Disposition", "attachement; filename=\"" + file.getName() + "\"")
                        .build();
            default:
